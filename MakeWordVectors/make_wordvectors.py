...
def make_wordvectors():
    ...
    sents = []
    with codecs.open('data/corpus/all.txt', 'r', 'utf-8') as fin:
        while 1:
            line = fin.readline()
            if not line: break
    ...
    model.save('data/bangla_word2vec_model/BengWord2VecModel.bin')
    
    # Save to file
    with codecs.open('data/corpus/posts.tsv', 'w', 'utf-8') as fout:
        for i, word in enumerate(model.wv.index2word):
            fout.write(u"{}\t{}\t{}\n".format(str(i), word.encode('utf-8').decode('utf-8'),
